...
pub struct PlatformToolsResolution {
    /// e.g. `"v1.56"`. Kept as a string because upstream uses the `v`-prefixed
...
/// Path where the given platform-tools `version` (e.g. `"v1.56"`) is installed.
...
mod tests {
    ...
    fn download_url_prepends_v_when_missing() {
        let with_v = download_url("v1.56");
        let without_v = download_url("1.56");
        assert_eq!(with_v, without_v);
        assert!(with_v.contains("/releases/download/v1.56/"));
    }

    #[test]
    fn download_url_targets_anza_platform_tools() {
        let url = download_url("v1.56");
    ...
    fn replace_install_dir_swaps_existing_target_after_staging_is_ready() {
        let dir = tempfile::TempDir::new().unwrap();
        let target = dir.path().join("v1.56");
        let staging = dir.path().join("v1.56.partial");
