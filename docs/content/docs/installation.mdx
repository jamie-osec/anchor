...
Install Anchor with the Anchor Version Manager (AVM). First, install Rust with
rustup:
...
Then install AVM and the latest nightly Anchor CLI:
...
<Callout type="warn">
  ...
  Dependencies](#install-dependencies)). Then run the commands above in the
...
The first `anchor` command installs the required Solana CLI and platform tools.

<Callout>

If the quick installation commands above don't work, refer to the [Install
Dependencies](#install-dependencies) section below for individual setup
instructions.

Install Node.js and Yarn from the individual instructions below when you need a
TypeScript-based test template.
...
Install AVM and the latest nightly Anchor CLI with the official installer:

```shell title="Terminal"
curl -sSfL https://raw.githubusercontent.com/otter-sec/anchor/master/avm/install | sh
```

The installer enables the nightly channel and links the `avm` and `anchor`
commands into `~/.cargo/bin` when possible. Check that AVM was installed
successfully:
...
To use a published Anchor release instead of nightly, disable the nightly
channel, then install and activate the version you need:
...
To install a specific published version, specify the version number:
